...
abstract class ExpressionComparer : IEqualityComparer<Expression>
{
    protected NameComparison CompareLambdaNames { get; set; }
    protected NameComparison CompareParameterNames { get; set; }
    protected ConstantComparison CompareConstants { get; set; }
            ...
            case ExpressionType.MultiplyAssignChecked:
                {
                    ...
                    return xt.Method == yt.Method 
                            && EqualsImpl(xt.Left, yt.Left, parameterContext) 
                            && EqualsImpl(xt.Right, yt.Right, parameterContext) 
                            && EqualsImpl(xt.Conversion, yt.Conversion, parameterContext);
                }
            ...
            case ExpressionType.Constant:
                {
                    switch(this.CompareConstants) {
                        ...
                        case ConstantComparison.ByExpressionReference:
                            return x == y;
                        case ConstantComparison.ByTypeOnly:
                            return true; //type equality is already confirmed
                        default:
